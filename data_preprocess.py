...
def convert_dcm(image_paths, output_path = 'MAIDA/data', PNG = False):
    for n, image in enumerate(image_paths):
        ...
        scaled_image = (np.maximum(pixel_array_numpy, 0) / pixel_array_numpy.max()) * 255.0
...
def downsize_anno(ori_ann_file='annotations.json',
    output_file='annotations_downsized.json',
    resized_dim=1280):
...
def get_stats(root="/home/ec2-user/segmenter/MAIDA/MIMIC_ETT_annotations", image_dir='ETTAnnotated'):
...
def get_reviewed_anno(ori_ann_file='/home/ec2-user/efs/MAIDA/MIMIC-1105-annotations.json',
...
def generate_gt_label( # xmid, ymid, w, h
    ...
    annotation_file='annotations.json',
    image_dir='yolo/images', out_dir='yolo/labels'):
    ...
    for file_path in os.listdir(os.path.join(root, image_dir)):
        ...
        for idx in ann_idx:
            ...
            bbox = [(xmin+w/2)/id_to_width[id], (ymin+h/2)/id_to_height[id], w/id_to_width[id], h/id_to_height[id]]
            out_file.write(" ".join([str(a) for a in (id_to_class[ann["category_id"]], *bbox)]) + '\n')
...
def make4classes(file='/home/ec2-user/segmenter/MAIDA/data1000/train_annotations.json',
